            ...
            $("#selectColonia").html(function () {
                ...
                for (var i in dataJson) {
                    contenido += "<datalist id='selectColonia'>"
                            + "<option value='" + dataJson[i].asenta + "'>" + utf8_decode(dataJson[i].asenta) + "</option> \n\
                    </datalist>";
                    $("#txtciudad").val(utf8_decode(dataJson[i].ciudad));
                    $("#txtestado").val(utf8_decode(dataJson[i].estado));
                }
                return contenido;
            });
        });
